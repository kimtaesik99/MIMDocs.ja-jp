---
title: MIM 証明書マネージャーの操作 |Microsoft Identity Manager
ms.custom:
  - Identity Management
  - MIM
...
3.  クリックして **ファイル & gt;スナップインの追加/削除**.;

4.  使用可能なスナップインの一覧でクリックして **証明書テンプレート** ] をクリックし、 **追加**します。

5.  MMC の **[コンソール ルート]** の下に、 **[証明書テンプレート]** が表示されます。 ダブル クリックすると、すべての利用可能な証明書テンプレートが表示されます。

6.  右クリックし、 **スマート カード ログオン** テンプレートとクリック **テンプレートの複製**します。
...
9.  **要求の処理** ] タブで、設定、 **目的** に **署名と暗号化** [ **、次の操作をしています.**  **[登録中にユーザーにメッセージを表示する]**を選択します。
...
13. 選択 **ファイル** ] をクリック **スナップインの追加と削除** 証明機関スナップインを MMC コンソールを追加します。 管理するコンピューターを尋ねられたら、 **[ローカル コンピューター]**を選択します。

14. MMC の左側のウィンドウで **[証明機関 (ローカル)]** を展開し、証明機関の一覧内に自身の CA を展開します。

15. 右クリック **証明書テンプレート**, 、クリックして **新規 & gt;証明書のテンプレート** 問題にします。
...
8.  チェック ボックスの横に **仮想スマート カードの作成/破棄** と **[管理者キーの**です。
...
8.  & Lt; Identity & gt;要素、たとえば、署名証明書に示されている件名と同じでなければ Publisher 属性の値を変更"CN = サブジェクト"です。
...
## アプリの展開
ダウンロード センターで CM アプリを設定すると、ファイル mimdmmodernapp _ & lt; バージョン & gt;_AnyCPU_Test.zip をダウンロードして、すべての内容を抽出します。 .appx ファイルはインストーラーです。 通常は展開する Windows ストア アプリを使用して任意の方法で展開することができます [System Center Configuration Manager](https://technet.microsoft.com/library/dn613840.aspx), 、または [Intune](https://technet.microsoft.com/library/dn613839.aspx) 、アプリのサイドローディングをユーザーがポータル サイトを通じてアクセスする必要がまたは自分のマシンに直接プッシュされますそれを取得するためです。
