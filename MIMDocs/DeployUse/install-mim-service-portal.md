...
最後の手順で、MIM インストール パッケージをセットアップしなかった場合は、これらの手順に従って Microsoft Identity Manager 2016 コンポーネントをインストールします。
...
6.  **[MIM データベース接続の構成]** ページで、**[新しいデータベースを作成]** を指定します。

    ![MIM データベース接続イメージを構成する](media/MIM-Install10.png)

7.  **[メール サーバーの接続の構成]** で、**[メール サーバー]** に Exchange サーバーの名前を入力します。 メール サーバーが構成されていない場合は、メール サーバー名として localhost を指定し、一番上の 2 つのチェック ボックスをオフにします。  **[次へ]**をクリックします。
...
![MIM サービスおよびポータルのインストールの画像](media/MIM-Install16.png)
