...
# 要塞環境の高可用性と障害復旧の考慮事項
この記事では、Privileged Access Management (PAM) に対し Active Directory ドメイン サービス (AD DS) と Microsoft Identity Manager 2016 (MIM) を展開する際の高可用性と障害復旧に関する考慮事項を説明します。
...
| 特権ロールのアクティブ化  | 中 | 専用のスマート カードを使用するアカウントで、管理グループにユーザーを手動で追加する |
...
要塞環境にある Active Directory ドメイン サービスをホストしているコンピューターと MIM サービスをホストしているコンピューターには、次に対する既存のフォレスト内のリソースへの双方向接続が必要です。
...
要塞環境のすべての機能に対する最小の高可用性トポロジは、少なくとも 4 台のサーバーと共有記憶域から構成されます。 2 台のサーバーは、ドメイン コントローラーとして構成し、Active Directory ドメイン サービスを提供する必要があります。 他の 2 台のサーバーは、SQL Server を提供するフェールオーバー クラスターとして構成して、MIM サービスを提供することができます。
...
- Active Directory ドメイン サービスの仮想化された展開には、「[仮想化ドメイン コントローラーの展開と構成](https://technet.microsoft.com/library/jj574223.aspx)」で説明されているように、仮想化プラットフォームの特定の機能が必要です。
...
Windows Server Active Directory ドメイン サービスには、ネイティブで高可用性と障害復旧のサポートが含まれています。
