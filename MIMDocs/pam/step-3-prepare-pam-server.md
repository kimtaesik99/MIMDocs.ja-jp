...
2.  次のコマンドを入力します。 .NET Framework 3.5 の機能のソース ファイルに対しては、別の場所を指定することが必要になる場合があります。 通常、Windows Server のインストール時にこれらの機能は表示されませんが、OS インストール ディスク ソース フォルダー (d:\Sources\SxS など) 上のサイド バイ サイド (SxS) フォルダーに存在します。\.
...
4. ファイルを保存し、PowerShell コマンドで IIS を再起動します `iisreset /START`
